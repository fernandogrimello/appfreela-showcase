# 🚀 AppFreela Platform

🔗 **Live Demo:**  
https://appfreela.onrender.com/

---

## 💼 Sobre o projeto

O **AppFreela** é uma plataforma fullstack que conecta **contratantes** a **profissionais autônomos**, permitindo a contratação de serviços de forma prática, segura e organizada.

O projeto está em **desenvolvimento ativo**, com melhorias contínuas e evolução das funcionalidades.

---

## ⚙️ Funcionalidades atuais

- 👤 Cadastro e autenticação de usuários
- 🔎 Busca de profissionais por serviço e localização
- 📅 Agendamento de serviços
- 💬 Chat entre contratante e profissional
- ⭐ Sistema de avaliações
- ❤️ Favoritar profissionais
- 📊 Dashboard com informações relevantes
- ⚖️ Sistema de contestação e mediação

---

## 🧠 Diferencial

O AppFreela foi projetado para simular um **fluxo real de marketplace**, incluindo:

**Solicitação → Negociação → Execução → Conclusão → Avaliação → Mediação**

Isso permite representar cenários reais de contratação de serviços.

---

## 🚧 Em desenvolvimento

Atualmente, o projeto está sendo aprimorado com foco em:

- Melhorias na experiência mobile
- Refatoração de código e organização estrutural
- Padronização das chamadas de API
- Evolução da arquitetura para maior escalabilidade
- Ajustes no sistema de notificações

---

## 🛠️ Tecnologias

**Back-end**
- Node.js
- Express
- MySQL / TiDB
- JWT
- Bcrypt

**Front-end**
- HTML5
- CSS3
- JavaScript

**Infraestrutura**
- Render (deploy)
- GitHub

---

## 🗄️ Arquitetura

O sistema foi estruturado com separação entre:

- Rotas
- Controllers
- Middlewares
- Banco de dados
- Front-end modular

---

## 🎯 Objetivo

Este projeto tem como objetivo:

- Aplicar conceitos de desenvolvimento fullstack
- Simular um sistema real de marketplace
- Evoluir continuamente como projeto de portfólio profissional

---

## ⚠️ Observação

Este repositório é uma **vitrine pública** do projeto.

O código-fonte completo está mantido em repositório privado.

---

## 📫 Contato

- GitHub: https://github.com/fernandogrimello  
- LinkedIn: https://www.linkedin.com/in/luiz-fernando-grimello-6568b4358/

---

## 👨‍🏫 Orientação

Projeto desenvolvido com orientação de:

- Hudson Neves (Instrutor - SENAC)  
  https://github.com/HudsonNeves  

**Instituição:**  
Centro de Educação Profissional José Rufino e Carlos Aguiar (SENAC)

## 📌 Licença

© 2026 Luiz Fernando Grimello. All rights reserved.  
Uso apenas para demonstração.
